...
Final 阶段计划整理一个自采拍屏视频测试集，用来覆盖更真实的输入变化。测试集设计为 5 类场景，每类 10 段，每段约 5 秒，共 50 段短视频：

| 类别 | 场景 | 测试目的 |
| --- | --- | --- |
| 静态网页/文档 | 屏幕内容基本不动，包含文字和浏览器 UI | 验证基础屏幕捕获、透视矫正和低抖动输出 |
| 滚动网页 | 页面滚动、鼠标移动或局部 UI 变化 | 测试屏幕内部运动是否干扰平面估计 |
| 屏幕内播放视频 | 屏幕区域内有大面积动态画面 | 测试动态内容是否错误拉动 homography |
| PPT/浅色页面 | 屏幕边框弱、内容区域大面积白底或无明显线条 | 测试低纹理和无明显边框场景下的稳定性 |
| 4K/摩尔纹/反光难例 | 高分辨率拍摄、摩尔纹、反光或轻微遮挡 | 连接 demoiréing 前处理叙事，并分析失败边界 |

Proposal 阶段不把这个 50 段测试集当作已经完成的结果；当前只展示一个可追溯样例上的初步消融。这个样例用于证明主流程能跑通，并在同一段输入、同一分辨率、同一帧数下比较三种几何轨迹来源：
...
评价指标定义如下，数值越低表示归一化输出越稳定：

| 指标 | 单位 | 含义 |
| --- | --- | --- |
| Residual translation p95 | px | 归一化后相邻帧残余仿射运动中的平移量 95 分位数，用来衡量输出画面的残余平移抖动 |
| Residual rotation p95 | degree | 相邻帧残余旋转角 95 分位数，用来衡量输出画面的残余旋转晃动 |
| Residual scale delta p95 | ratio | 相邻帧尺度变化量 95 分位数，用来衡量输出画面的残余缩放抖动 |
| Tracker accept ratio | ratio | RANSAC 和几何门控接受的帧比例，用来判断跟踪是否频繁失败 |
| RANSAC inliers / inlier ratio | count / ratio | 单应估计中的内点数量和比例，用来衡量特征匹配是否可靠 |
| Feature coverage | ratio | 跟踪特征点在屏幕平面内的横向和纵向覆盖范围，用来避免特征只集中在局部区域 |

报告里的主要量化表优先展示 residual translation、rotation 和 scale delta；tracker accept ratio、inliers 和 coverage 用于解释成功或失败原因。
...
- Residual translation p95、rotation p95 和 scale delta p95 用于衡量输出视频的残余晃动，数值越低越稳定。
- Tracker accept ratio、RANSAC inliers 和 feature coverage 用于解释跟踪是否可靠，以及失败来自检测、匹配还是动态内容干扰。
