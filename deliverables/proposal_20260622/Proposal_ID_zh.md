...
**Group ID:** TODO | **成员:** 温镕硕 Rongshuo Wen (Leader, 124020369), 温璧华 Bihua Wen (124090670), 刘明睿 Ruiming Liu (124090375)

## 问题背景与动机

现有拍屏去摩尔纹数据集证明了屏幕恢复需求真实存在，但它们通常面向受控采集、裁剪、配对或已经时空对齐的屏幕内容。真实手机拍屏视频还包含屏幕外背景、透视倾斜、手持晃动、弱边框和屏幕内部动态内容。本项目关注的是进入去摩尔纹、OCR 或归档之前缺失的前置几何归一化链路。
...
方法首先在首帧自动检测或手动指定屏幕四角点，并用 homography 将屏幕透视矫正到 16:9 画布。为避免逐帧角点检测带来的抖动，系统在参考平面上使用 Lucas-Kanade 光流跟踪特征点，并通过 RANSAC 估计鲁棒单应变换。几何门控用于拒绝不可靠更新，之后对轨迹进行插值和平滑，最后生成归一化输出视频。
...
| 参考平面跟踪 | **0.118 px** | **0.0044 deg** | 残余稳定性最好 |
...
源文件和证据保存在 `deliverables/proposal_20260622/`。当前草稿保留 Group ID 占位，拿到正式组号后再替换。
