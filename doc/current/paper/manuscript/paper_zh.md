...
为避免把历史诊断项混入主实验，消融只保留当前项目实现中可直接切换的模块（表 4）。实验在代表性滚动片段上使用相同初始化、输出画布和指标代码。表中先依次去掉 LK 一致性诊断和物理边框证据，再固定其余设置，仅替换边框观测方式。
...
这个消融支持两个判断。第一，LK/RANSAC 在当前方法中不是主要几何驱动；关闭 LK 诊断后结果不变，说明该片段的四边形由边框证据确定。第二，物理边框线索是决定性模块；去掉边框后，相邻帧光流会跟随连贯滚动内容，角点 RMSE 升至 76.114 px。边框观测方式的替换进一步说明 profile 采样是更稳妥的默认选择：LSD 结果接近但略差，Hough 在该片段上产生明显几何退化。
