...
本文有四个有边界的贡献。第一，建立从完整场景视频和稀疏四角标注到矫正输出、CSV/JSON 指标和逐 clip 审核报告的可复现实验流程。第二，实现带显式更新接受诊断的参考帧锚定跟踪器。第三，在 14985 帧和更新后的 199 个非初始化标注帧上报告受控 first-pass 对比。第四，将标注几何、轨迹派生变化、局部细节保持和 FFT 方向诊断拆开报告，避免把平滑误读为正确。
...
每个视频的第 0 帧角点用于初始化，因此不进入几何误差统计。人工标注包含可见屏幕四角，顺序为左上、右上、右下、左下。当前 50 个视频共有 248 个标注帧；排除初始化帧后，全部 50 个视频都有非初始化几何标注，共 199 个可评分标注帧。补齐标注后，本文新增每类 2 个 clip 的轻量重算，用于更新几何和时域证据；细节和频域诊断仍作为 first-pass 辅助证据。
...
人工审核识别出三类代表失败（图 8）。第一，困难视角或遮挡会让早期几何误差被传播；`hard_01` 在正式运行中只接受 3/300 帧。第二，弱边界或低纹理场景导致可靠覆盖不足；`weak_border_10` 只接受 2/300 帧。第三，滚动内容会产生与物理屏幕无关的参考特征，因此即使已有非初始化几何标注，更新接受率仍可能偏低。
...
本实验仍有局限。数据集规模较小且由项目组自采集，不能证明跨设备、显示技术、拍摄距离或光照条件的泛化。几何标注仍是稀疏关键帧，而不是逐帧密集标注，尽管当前每个 clip 都已有非初始化几何标注。当前时域指标来自估计轨迹本身，不是独立物理稳定性证据。细节和频域指标没有配对干净录屏，因此不能评估去摩尔纹质量。最后，每次透视变换都会重采样屏幕内容，正面几何和较低抖动可能以模糊、振铃或高频结构变化为代价。
