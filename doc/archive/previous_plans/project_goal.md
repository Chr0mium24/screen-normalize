...
> 说明：本文件记录当前主线目标。项目定位已经从单纯几何 demo 调整为“真实拍屏视频恢复的前置屏幕捕获矫正与时域稳定链路”。SuperPoint/LightGlue 等学习式特征匹配只作为可选对照或 future work，不作为主线必做项。

## 题目

**面向真实场景拍屏视频恢复的屏幕捕获矫正与时域稳定化**

本项目面向数字图像处理课程大作业。目标不是训练一个完整图像恢复模型，也不是直接解决去摩尔纹、颜色校正或细节恢复，而是用传统图像处理和几何视觉方法，把手持拍摄电脑屏幕的视频先转换成接近正常录屏视角、几何稳定的屏幕内容视频。

这个结果可以作为后续 video demoiréing、OCR、归档或人工查看的前置输入。
...
从应用端链路看，现有拍屏图像和视频去摩尔纹方法往往依赖受控采集、裁切或已对齐的屏幕内容；真实用户视频首先需要解决屏幕检测、透视归一化和时域稳定。本项目补的是这段前置链路。
...
5. **轨迹平滑**  
   对角点轨迹再做离线几何门控，拒绝面积或边长突变的观测；对被门控拒绝的帧先用前后可靠帧插值，再对角点或运动参数序列做时域平滑，降低高频估计噪声。报告中可以把这部分解释为运动轨迹低通滤波。
...
SuperPoint + LightGlue 等学习式匹配可以作为 homography 估计的可选探针，用于和传统 LK tracker 对比，但它不是本项目当前主线，也不承担去摩尔纹或画质恢复任务。
...
从应用链路角度，期望结果还包括：为后续 video demoiréing 或 OCR 提供更稳定、更接近正面屏幕坐标系的输入。
...
参考文献集中在 `doc/paper/references/papers/`。
...
应用端叙事和与 video demoiréing 论文的关系见 `doc/paper/draft_materials/positioning.md`。其中去摩尔纹论文用于说明后续恢复模块和数据对齐背景，`Cinematic-L1 Video Stabilization with a Log-Homography Model` 用于支撑 homography 轨迹稳定化的理论表述。
