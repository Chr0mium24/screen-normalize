...
## Figure 2: Dataset Overview（数据集概览）

**内容：** 用自动统计和示例帧说明数据集覆盖范围
- **Fig. 2a:** 5 个场景类别的样本分布（柱状图：x 轴是类别，y 轴是视频数量/帧数）
- **Fig. 2b:** 视频时长、分辨率和帧率的基础统计表
- **Fig. 2c:** 每个类别选 1 个代表帧，直观展示场景差异
- **Fig. 2d:** 关键帧标注示例（从每个类别选 1 帧，展示标注的四个角点）

**数据来源：**
- 代码从 50 个视频自动读取的类别、时长、分辨率和帧率
- 关键帧标注文件（角点坐标）

不建立设备、光照、真实拍摄角度或场景难度的结构化标注。必要的场景特征由作者在定性分析和失败案例中直接讨论。
...
| Fig. 2 | 数据集概览    | 50 个视频的自动基础统计  | 5 个场景类别的分布  |
