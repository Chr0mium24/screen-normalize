...
本目录收集“拍屏视频屏幕捕获矫正与时域稳定化”方向的可引用论文。当前项目不直接做去摩尔纹、颜色校正或深度画质恢复，而是把传统几何方法作为真实拍屏视频恢复的前置链路：先检测屏幕、透视归一化并稳定，再为后续 video demoiréing、OCR 或归档提供对齐输入。
...
1. 现有拍屏去摩尔纹论文证明 screen restoration 是真实问题，但真实应用中首先需要把完整拍摄画面中的屏幕内容捕获和对齐。
2. 拍屏幕视频可以近似为“相机拍摄平面目标”，因此适合用四角点和单应变换做几何归一化。
3. 屏幕边界、文档边界、白板边界都属于相似问题，可以借鉴文档/白板矫正方法。
4. 逐帧检测角点容易抖动，所以要用特征跟踪、RANSAC 和轨迹平滑。
5. 屏幕上的水平/竖直结构可以作为线段和消失点证据，但不能被动态文字或视频内容逐帧驱动。
6. 稳定化本质是估计相机运动轨迹并滤除高频抖动，最终输出固定比例的正面屏幕视频。

应用端叙事和 video demoiréing 论文对应关系见 `../application-pipeline-story.md`。
